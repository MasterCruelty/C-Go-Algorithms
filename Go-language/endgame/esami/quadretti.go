 ...
 * 2. Se c è la casella in basso a destra ed è bianca, allora f(c) = 0 ==> sono già in basso a destra.
 ...
 *    - f(c) = 0 => non è possibile andare più a destra 
 ...
 ******************************************************************
 * Progettazione-descrizione camminiCompleti2:
...
func stampaMat(M [][]int) {
	for i := 0; i < len(M); i++ {
		fmt.Println(M[i])
	}
}

func camminiCompleti2(grid [][]int) int {
	rows := len(grid)
	cols := len(grid[0])
